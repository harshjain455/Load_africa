...
import { ShieldAlert, CheckCircle2, UploadCloud, AlertCircle, FileText } from 'lucide-react';
...
export default function FleetCompliance() {
  ...
  const handleFileChange = (e, key) => {
    // Mocking file upload by just setting a dummy string
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fleetService.submitCompliance({ company_documents: docs });
      if (res.success) {
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-fadeIn">
      <div>
        ...
        <p className="text-xs text-slate-500 font-medium">LoadAfrica requires strict compliance to activate your fleet.</p>
      </div>
      ...
      {status === 'ACTIVE' && (
        <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-4">
          ...
          <div>
            <h3 className="font-bold text-emerald-800 text-lg">Fleet Account Active</h3>
            <p className="text-emerald-700 text-sm mt-1">Your company has been verified and you are fully operational.</p>
          </div>
        </div>
      )}
      ...
      {status === 'UNDER_REVIEW' && (
        <div className="p-6 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-amber-700 text-sm mt-1">Your fleet account is currently under compliance review. The Admin team is verifying your company, vehicles, and drivers. You will be notified once approved.</p>
          </div>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-700">Goods in Transit Insurance</label>
