...
import { ArrowLeft, User, Phone, Mail, Activity, Calendar, Briefcase, FileText, CreditCard, RefreshCcw, Building } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64 text-slate-500">
        <RefreshCcw className="h-6 w-6 animate-spin mr-2" /> Loading broker details...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Broker Details</h1>
          ...
          <div className="space-y-6">
            ...
            {user.broker && (
              <div>
                ...
                <div className="p-4 bg-slate-50 rounded-xl space-y-2">
                  <p className="text-sm text-slate-600"><span className="font-semibold text-slate-900">Company Name:</span> {user.broker.company_name || 'N/A'}</p>
                  <p className="text-sm text-slate-600"><span className="font-semibold text-slate-900">Tax Number:</span> {user.broker.tax_number || 'N/A'}</p>
                </div>
              </div>
            )}
          </div>
