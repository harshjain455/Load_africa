...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if ((role === 'Fleet' || role === 'Broker' || role === 'Plant Owner') && !companyName) {
      setError('Company Name is required for Fleet, Broker and Plant Owner accounts.');
      return;
    }
    if (role === 'Driver' && !license) {
      setError('Driver License Number is required for Drivers.');
      return;
    }
    setError('');
    setLoading(true);
    ...
    try {
      await authService.register({
        ...
        lastName,
        phone,
        companyName: (role === 'Fleet' || role === 'Broker' || role === 'Plant Owner' || role === 'Customer') ? companyName : undefined,
    ...
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col items-center justify-center font-sans p-6 selection:bg-amber-500 selection:text-slate-955">
      ...
      <div className="bg-white border border-slate-200/80 shadow-md p-6 rounded-2xl w-full max-w-md space-y-4 text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-left">
          ...
          {/* Driver License input field (Conditional) */}
          {role === 'Driver' && (
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-700">Driver License Number</label>
              <input 
                type="text" 
          ...
          {/* Company Name input field (Conditional) */}
          {(role === 'Fleet' || role === 'Broker' || role === 'Plant Owner' || role === 'Customer') && (
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-700">Company Name {role === 'Customer' ? <span className="text-slate-400 font-normal">(Optional)</span> : ''}</label>
              <input 
                type="text" 
                placeholder={role === 'Customer' ? "Optional for individual customers" : "Your Company Ltd"}
                value={companyName}
