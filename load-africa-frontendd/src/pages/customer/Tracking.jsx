...
export default function Tracking() {
  ...
  if (!load || !driver) {
      ...
      <div className="max-w-4xl mx-auto py-12 text-center bg-white rounded-3xl border border-slate-200 shadow-sm p-8 space-y-4">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed font-light">
            You do not have any bookings actively in transit. Accept a quotation and secure a transporter to begin live tracking.
          </p>
        </div>
    ...
    // Only use real coordinates. Do not hardcode fallbacks.
  const startLat = load.pickup_coords_lat || null;
  const startLng = load.pickup_coords_lng || null;
  const endLat = load.delivery_coords_lat || null;
  const endLng = load.delivery_coords_lng || null;
