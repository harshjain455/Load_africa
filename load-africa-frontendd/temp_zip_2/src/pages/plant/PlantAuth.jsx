...
export default function PlantAuth() {
  const [email, setEmail] = useState('plant@loadafrica.co.za');
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate('/plant-portal/dashboard'); }, 800);
  };
      ...
      <div className="bg-white border border-slate-200/80 shadow-md p-10 rounded-2xl w-full max-w-md space-y-6 text-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="mt-3 space-y-1">
            ...
            <p className="text-xs text-slate-400 font-bold">Sign in to your yellow plant account</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <div className="space-y-1">
            ...
            <input type="email" placeholder="plant@example.com" value={email} onChange={e => setEmail(e.target.value)} required
        ...
        <div className="text-xs text-slate-500 font-bold pt-2">
          ...
          <button onClick={() => navigate('/plant/register')} className="text-[#f99c00] hover:text-[#e08b00] underline font-black transition-colors">List Your Plant</button>
        </div>
