...
import { 
  User, Settings, Shield, Bell, CreditCard, LogOut, CheckCircle2, Lock, Star
} from 'lucide-react';
...
export default function DriverProfile() {
  ...
  useEffect(() => {
    ...
    const d = drivers[0]; // Sipho Zuma
    setDriver(d);
    if (d) {
      setProfileData({
        name: d.name || 'Sipho Zuma',
        email: d.email || 'sipho@example.com',
        phone: d.phone || '+27 82 123 4567'
      });
    }
  }, []);
  ...
  const handleSaveProfile = (e) => {
    ...
    setTimeout(() => {
      ...
      const drivers = getMockData('drivers') || [];
      if (drivers.length > 0) {
        drivers[0] = { ...drivers[0], ...profileData };
            ...
            {activeTab === 'profile' && (
              <form onSubmit={handleSaveProfile} className="space-y-6">
                <div className="flex items-center gap-6 pb-6 border-b border-slate-100">
                  <img 
                    src={driver.avatar || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&auto=format&fit=crop&q=80'} 
                    alt="Driver" 
